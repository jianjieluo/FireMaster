...
void BaseTank::setBulletCount(const int n) {
    bullet_count = n;
}
void BaseTank::setSide(const int t_side) {
    m_side = t_side;
}
const int BaseTank::getSide() const {
    return m_side;
}
...
void BaseTank::runAttack() {
    ...
    if (bullet_count == 1) {
        auto s = Sequence::create(this->getAttackAnimate(), fireAnimate, launch, DelayTime::create(0.5f),
            this->getAfterAttackAnimate(), setDefaultLamda, nullptr);
        this->runAction(s);
    }
    else if (bullet_count == 3) {
        auto s = Sequence::create(this->getAttackAnimate(), fireAnimate, launch,
            DelayTime::create(0.5f), fireAnimate, launch, DelayTime::create(0.5f),
            fireAnimate, launch, DelayTime::create(0.5f), this->getAfterAttackAnimate(),
            setDefaultLamda, nullptr);
        this->runAction(s);
    }
}
