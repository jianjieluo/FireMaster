...
void MciPlayer::Open(const char* pFileName, UINT uId) {
  ...
  do {
    ...
    //         BREAK_IF(! pBuf);
    //         MultiByteToWideChar(CP_ACP, 0, pFileName, nLen + 1, pBuf, nLen +
    //         1);
...
void MciPlayer::Resume() {
  if (strExt == ".mid") {
    ...
    _SendGenericCommand(
        ...
        reinterpret_cast<DWORD_PTR>(&mciStatusParms));  // MCI_STATUS
    mciPlayParms.dwFrom = mciStatusParms.dwReturn;      // get position
    _SendGenericCommand(MCI_PLAY, MCI_FROM, reinterpret_cast<DWORD_PTR>(
                                                &mciPlayParms));  // MCI_FROM
  } else {
...
// private member
//////////////////////////////////////////////////////////////////////////
void MciPlayer::_SendGenericCommand(int nCommand, DWORD_PTR param1 /*= 0*/,
                                    DWORD_PTR parma2 /*= 0*/) {
  if (!_dev) {
    return;
  }
